...
class Config:
    LOG_FORMAT = (
        "%(asctime)s.%(msecs)03d [%(levelname)s] %(module)s.%(funcName)s:%(lineno)d (%(process)d:"
    ...
    ALL_UNEDITABLE_SCHEMAS = (
        os.getenv("ALL_UNEDITABLE_SCHEMAS", "false") == "true"
    )  # type: bool
    ...
    PREVIEW_CLIENT_USERNAME = os.getenv(
        "PREVIEW_CLIENT_USERNAME"
    )  # type: Optional[str]
    PREVIEW_CLIENT_PASSWORD = os.getenv(
        "PREVIEW_CLIENT_PASSWORD"
    )  # type: Optional[str]
    PREVIEW_CLIENT_CERTIFICATE = os.getenv(
        "PREVIEW_CLIENT_CERTIFICATE"
    )  # type: Optional[str]
    ...
    ANNOUNCEMENT_CLIENT_ENABLED = (
        os.getenv("ANNOUNCEMENT_CLIENT_ENABLED") == "true"
    )  # type: bool
    # Maps to a class path and name
    ...
    DASHBOARD_PREVIEW_FACTORY = (
        None  # By default DefaultPreviewMethodFactory will be used.
    )
...
class LocalConfig(Config):
    ...
    JS_CONFIG_OVERRIDE_ENABLED = distutils.util.strtobool(
        os.environ.get("JS_CONFIG_OVERRIDE_ENABLED", "False")
    )

    FRONTEND_BASE = os.environ.get(
        "FRONTEND_BASE",
        "http://{LOCAL_HOST}:{PORT}".format(LOCAL_HOST=LOCAL_HOST, PORT=FRONTEND_PORT),
    )

    SEARCHSERVICE_BASE = os.environ.get(
        "SEARCHSERVICE_BASE",
        "http://{LOCAL_HOST}:{PORT}".format(LOCAL_HOST=LOCAL_HOST, PORT=SEARCH_PORT),
    )

    METADATASERVICE_BASE = os.environ.get(
        "METADATASERVICE_BASE",
        "http://{LOCAL_HOST}:{PORT}".format(LOCAL_HOST=LOCAL_HOST, PORT=METADATA_PORT),
    )
