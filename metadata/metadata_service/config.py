...
PROXY_CLIS = {"MYSQL": "metadata_service.cli.rds_command.rds_cli"}
...
class Config:
    ...
    SWAGGER = {"openapi": "3.0.2", "title": "Metadata Service", "uiversion": 3}
...
class LocalConfig(Config):
    ...
    PROXY_ENCRYPTED = bool(
        distutils.util.strtobool(os.environ.get(PROXY_ENCRYPTED, "True"))
    )
    PROXY_VALIDATE_SSL = bool(
        distutils.util.strtobool(os.environ.get(PROXY_VALIDATE_SSL, "False"))
    )
...
class MySQLConfig(LocalConfig):
    ...
    SQLALCHEMY_DATABASE_URI = os.environ.get(
        "SQLALCHEMY_DATABASE_URI", "mysql://user:password@127.0.0.1:3306/amundsen"
    )
...
try:
    ...
    class NeptuneConfig(LocalGremlinConfig, LocalConfig):
        ...
        PROXY_PASSWORD = boto3.session.Session(
            region_name=os.environ.get("AWS_REGION", "us-east-1")
        )

        PROXY_CLIENT_KWARGS = {
            "neptune_bulk_loader_s3_bucket_name": os.environ.get("S3_BUCKET_NAME"),
            "ignore_neptune_shard": distutils.util.strtobool(
                os.environ.get("IGNORE_NEPTUNE_SHARD", "True")
            ),
    ...
    logging.warning(
        """amundsen_gremlin not installed. GremlinConfig and NeptuneConfig classes won't be available!
